...
モジュール名は C<Zengin::Pl>、ディストリビューション名は C<Zengin-Pl> です。
C<Zengin::Client> は後方互換のために残しています。

=head1 INSTALLATION

=head2 cpanm でローカル clone をインストール

    git clone https://github.com/sironekotoro/zengin-pl.git
    cd zengin-pl
    cpanm .

C<cpanm /path/to/zengin-pl> のように、clone 済みディレクトリを直接指定しても
インストールできます。

=head2 標準的な Build.PL 手順
...
C<base_url> のデフォルト値は
L<https://raw.githubusercontent.com/sironekotoro/zengin-data-mirror/main/data>
です。

=head1 META

C<meta()> は backend 自身の情報をハッシュリファレンスで返します。
...
C<base_url> は現在実際に使っている値を返します。
C<source.revision> と C<source.updated_at> は将来拡張用の枠として持っていますが、
現時点では未実装のため C<undef> を返します。
...
既存コード向けに C<Zengin::Client> も引き続き利用できますが、
新規利用では C<Zengin::Pl> を使ってください。

=head1 DEVELOPMENT

このディストリビューションは Minilla で管理しています。C<minil.toml> を設定の正とし、
C<Build.PL> や C<META.json> のような生成物は手で編集せず Minilla で更新してください。

C<META.json> は Git clone 後に C<cpanm /path/to/cloned-repo> を成立させるため、
生成物ですがリポジトリにも含めています。

メンテナ向けの個人運用スクリプトについては C<author/README.md> を参照してください。
